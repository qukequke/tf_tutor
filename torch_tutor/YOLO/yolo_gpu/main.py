...
def train():
    ...
    for epoch in range(opt.num_epochs):
        ...
        for param_group in optimizer.param_groups: #每一层可以设置不同的学习率
...
def predict():
    ...
    imgs = sorted(glob.glob(test_img_dir))
    print(imgs)
    for test_img_dir in imgs: 
        ...
        for left_up, right_bottom, class_name, _, prob in result:
            ...
            print(prob)
        # 将测试结果写入
        img_name = os.path.basename(test_img_dir)
        cv2.imwrite(os.path.join(opt.result_img_dir, img_name),image)
