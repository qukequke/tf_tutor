...
class yoloLoss(nn.Module):
    ...
    def forward(self,pred_tensor,target_tensor):
        '''
        pred_tensor: (tensor) size(batchsize,S,S,Bx5+20=30) [x,y,w,h,c] [None, 7, 7, 30]
        target_tensor: (tensor) size(batchsize,S,S,30)  [None, 7, 7, 30]
        ...
        print('coo_mask shape is ' + str(coo_mask.shape))
        print('pred_tensor shape is ' + str(pred_tensor.shape))
        print('target_tensor shape is ' + str(target_tensor.shape))
        ...
        # noo pred只需要计算类别c的损失
        noo_pred_c = noo_pred[noo_pred_mask] #只取出两个概率值
        ...
        for i in range(0,box_target.size()[0],2):
            box1 = box_pred[i:i+2] #box_pred [136, 5] box1[2, 5] box_target [136, 5]
            #选中两行 这两行第两个框 之前reshape的
            box1_xyxy = Variable(torch.FloatTensor(box1.size())) #[2, 5]
            box1_xyxy[:,:2] = box1[:,:2] -0.5*box1[:,2:4] #由[xc, yc, w, h] -> [x1, y1, x2, y2]
            box1_xyxy[:,2:4] = box1[:,:2] +0.5*box1[:,2:4]
            box2 = box_target[i].view(-1,5) #[1, 5]
            box2_xyxy = Variable(torch.FloatTensor(box2.size()))
            box2_xyxy[:,:2] = box2[:,:2] -0.5*box2[:,2:4]
            box2_xyxy[:,2:4] = box2[:,:2] +0.5*box2[:,2:4]
            iou = self.compute_iou(box1_xyxy[:,:4],box2_xyxy[:,:4]) #[2,5] , [1, 5] return [2, 1]
