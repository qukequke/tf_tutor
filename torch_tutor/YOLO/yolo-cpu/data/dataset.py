...
class yoloDataset(data.Dataset):
    ...
    def __init__(self,root,list_file,train,transform):
        ...
        for line in lines:
            ...
            for i in range(num_faces):
                ...
                c = splited[6+5*i]
                box.append([x,y,x2,y2]) #存放一张图片中的所有物体
                label.append(int(c)+1)
            # bbox  写入所有物体的坐标值
            self.boxes.append(torch.Tensor(box)) #存放所有图片 [[list1], [list2]]torch.Tensor会变成2列
            # label 写入标签
    ...
    def __getitem__(self,idx):
        ...
        h,w,_ = img.shape
        boxes /= torch.Tensor([w,h,w,h]).expand_as(boxes) #在这做了归一化 变成0,1之间
    ...
    def encoder(self,boxes,labels):
        ...
        for i in range(cxcy.size()[0]):
            ...
            target[int(ij[1]),int(ij[0]),2:4] = wh[i]  #一个格子内的两个框设置一样
