...
class DataAugmentForObjectDetection():
    ...
    def dataAugment(self, img, bboxes):
        ...
        for i in range(1):
            # if random.random() < 0.0:  # self.crop_rate:        #裁剪
            if random.random() < 1:  # self.crop_rate:        #裁剪
                print('裁剪')
                change_num += 1
                img, bboxes = self._crop_img_bboxes(img, bboxes)
                # self.save_img(img, bboxes)
            ...
            # if random.random() < self.cutout_rate:  # cutout
                # print('cutout')
                change_num += 1
                img = self._cutout(img, bboxes, length=200, n_holes=3, threshold=0.5)
    ...
    def save_img(self, img, box, name):
        draw = ImageDraw.Draw(img)
        draw.rectangle(box, outline='red')
        # pil_img.show()
        pil_img.save(os.path.join('my_test_dir', name + '.jpg'))
...
if __name__ == '__main__':
    ...
    data_auger = DataAugmentForObjectDetection()
    img2, bboxes = data_auger.dataAugment(img, [[0, 0, 500, 500]])
    pil_img = Image.open('timg.jpg')
    ...
    ax[0].imshow(img)
    ax[1].imshow(np.array(img2, dtype=np.uint8))
    plt.show()
